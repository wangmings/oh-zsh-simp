...
module ColorLS
  ...
  class Core
    ...
    def ls
      ...
      @theme = ENV['ColorLS_THEME']

      
      
      # 通过显示宽度、设置显示文件列数
      screen_width = ColorLS.screen_width + 30
    ...
    def init_icons
      ...
      @files          = ColorLS::Yaml.new('yaml', 'files.yaml').load
      @file_aliases   = ColorLS::Yaml.new('yaml', 'file_aliases.yaml').load(aliase: true)
      @folders        = ColorLS::Yaml.new('yaml', 'folders.yaml').load
      @folder_aliases = ColorLS::Yaml.new('yaml', 'folder_aliases.yaml').load(aliase: true)
      
      # 新增文件别名 hello
      @new_icons_color  = ColorLS::Yaml.new('yamls', 'color_font_icons.yaml').load
      @new_file_name    = ColorLS::Yaml.new('yamls', 'new_file_name.yaml').load(aliase: true)
      @new_file_alias   = ColorLS::Yaml.new('yamls', 'new_file_alias.yaml').load(aliase: true)
      @new_folder_alias = ColorLS::Yaml.new('yamls', 'new_folder_alias.yaml').load(aliase: true)
    ...
    def user_info(content)
      ...
      # 修改新增 hello
      @userlength = @userlength == 4 ? 3 : @userlength
      owner = content.owner.rjust(4, ' ')

      owner.ljust(@userlength, ' ').colorize(@colors[:user])
    ...
    def size_info(filesize)
      ...
      size = "#{size[0][0..-4].rjust(4,' ')} #{size[1].ljust(2,' ')}"
      
      # 修改新增
      size = size.gsub('KiB', 'K').gsub('MiB', 'M').gsub('GiB', 'G')
      size = size.split(" ") 
      size[0] = size[0].rjust(3, ' ')
      size = size.join(" ")
      # print size
    ...
    def mtime_info(file_mtime)
      ...
      # 修改部分 hello
      datetime = DateTime.parse(mtime)
      mtime = datetime.strftime('%b %-d %H:%M')
      mtime = mtime.split(" ") 
      mtime[1] = mtime[1].rjust(2, ' ')
      mtime = mtime.join(" ")
    ...
    # 十六进制颜色转换成RGB颜色 hello
    def hex_rgb_num(hex_color)
      red = hex_color[1..2].hex
      green = hex_color[3..4].hex
      blue = hex_color[5..6].hex 
      "#{red};#{green};#{blue}"
    ...
    # 组装新的文件图标
    def package_file_icons(keys, name)
      
      icons = @new_icons_color[keys]
      icons = icons.split("|") 
      icons_char = icons[0].sub(' ','')
      icons_color = hex_rgb_num(icons[2])
      icons_char = icons_char.gsub(/\\u[\da-f]{4}/i) { |m| [m[-4..].to_i(16)].pack('U') }
      
      # 设置大图标  
      # "\033[38;2;#{icons_color}m#{icons_char} \033[0m"
      "\033[38;2;#{icons_color}m#{icons_char}\033[0m "
    ...
    # 组装图标和文件名
    def fetch_string(content, key, keys, color, increment)
      @count[increment] += 1
      ...
      if @theme == "logo"
        ...
        # 新的文件图标
        file_icons = package_file_icons(keys, name)
        name = "#{name.colorize(color)}"
        entry = "#{out_encode(file_icons)}#{out_encode(name)}"
        # puts entry

        "#{inode(content)} #{long_info(content)} #{git_info(content)} #{entry}#{symlink_info(content)}"
    ...
    def options(content)
      ...
      if content.directory?
        ...
        # 新增
        keys = @new_folder_alias[keys]
        keys = @new_folder_alias[key] if keys.nil?
        keys = :dir if keys.nil?
        # puts keys
        ...
        name = content.name
        key = keys = File.extname(content.name).delete_prefix('.').downcase.to_sym
        
        # 新增
        keys = @new_file_alias[keys]
        keys = @new_file_name[name] if keys.nil?
        keys = :file if keys.nil?
