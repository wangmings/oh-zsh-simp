...
-ftb-complete() {
  ...
  case $#_ftb_complist in
    ...
    *)
      ...
      # 添加部分 -----------------------------------------------------
      local file_list file_arr ftb_arr ftb_complist file_type=0 _char file_path=`pwd`

      for paths in $(echo $BUFFER); do
        if [[ $paths =~ / ]];then
          [[ -d $paths ]] && file_path=$paths
          break 
        fi
      done
      ...
      ftb_complist=("${(@)${(@)_ftb_complist%$nul*}#*$nul}")
      file_arr=(`echo $file_list`)
      ftb_arr=(`echo $ftb_complist`)
      

      if (( $#file_arr == $#ftb_arr )) && [[ $ftb_complist =~ $ftb_arr[1] ]];then
        file_type=1
        _ftb_complist=$(logo-ls -1 $file_path)
      fi


      # 增加Unicode字符编码处理
      my-unicode-char(){
        echo $1|xxd -p|awk -F "e2a080" "{print \$2}"|xxd -r -p|sed "s/ *$//g;s/\///g"
      }
      ...
      # 添加部分 -----------------------------------------------------
      if (( $file_type == 1 && $#choices > 1 ));then
        _char=$(my-unicode-char $choices[3])
        choices[3]="$nul$_char$nul"
      fi

      unset -f my-unicode-char
      # 添加部分 -----------------------------------------------------


      # echo "$choices[1]|$choices[2]|$choices[3] || $#choices[*] |$choices[*]" > ~/vv2.txt
